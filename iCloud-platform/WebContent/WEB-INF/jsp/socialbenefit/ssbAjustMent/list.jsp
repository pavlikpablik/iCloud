...
<%-- 		    <c:choose>
			...
			<tr>
				...
				<td align="center">
					<a class="btn btn-mini  page_detail" href="ssbajustment_detail.action?candAjustMent.id=${entry.id}" >详细</a>
					<c:if test="${entry.ajustmentStatus == 0 || entry.ajustmentStatus == 3}">
						<a class="btn btn-mini  page_edit" href="ssbajustment_edit.action?candAjustMent.id=${entry.id}" >修改</a>
					</c:if>
					<c:if test="${entry.ajustmentStatus == 0}">
						<a class="btn btn-mini  page_status" href="ssbajustment_changeStatus.action?candAjustMent.id=${entry.id}&candAjustMent.ajustmentStatus=1">提交</a>
					</c:if>
					<c:if test="${entry.ajustmentStatus == 0 || entry.ajustmentStatus == 3}">
						<a class="btn btn-mini  page_status" href="ssbajustment_delete.action?candAjustMent.id=${entry.id}">删除</a>
					</c:if>
				</td>
			  </tr>
		</c:forEach>
...
<!-- 分页类  -->
<!-- 分页 -->
<c:set var="url" value="ssbajustment_prepaging.action" scope="request"/>
