	...
	<div class="span10 ">
		...
		<div class="btn-toolbar list_bottoms">
			<div class="btn-group">
				...
				<a id="page-create" class="btn page_edit" href="socialBenefitBase_edit.action"><i class="icon-pencil"></i>新建</a>
				</c:if>
			...
			<%-- <div class="btn-group">
				<c:if test="${method == 'manage' || method == 'create' || method == 'modify' }">
					<AT:AuthorityTag actionName="socialBenefitBase_importfile.action">
					<a class="btn page_import" url="socialBenefitBase_importpre.action?method=${method}" href="#"><i class="icon-upload"></i>导入</a>
					</AT:AuthorityTag>
				</c:if>
				<c:if test="${method == 'manage' || method == 'modify' || method == 'view'}">
					<AT:AuthorityTag actionName="socialBenefitBase_export.action">
						<a class="btn page_export" url="socialBenefitBase_export.action" href="#"><i class="icon-download"></i>导出</a>
					</AT:AuthorityTag>
				</c:if>
			</div> --%>
			<c:if test="${method == 'audit'}">
				<div class="btn-group">
					<a id="page-batchAudit" class="btn" url="socialBenefitBase_batchAudit.action" ><i class="icon-check"></i>批量审批</a>
				</div>
			</c:if>
		</div>
