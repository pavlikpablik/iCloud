...
<table  class="table_nowrap table table-condensed">
			...
			<tbody id="socialBenefitImport">
				...
				<tr >
					...
					<td >
						<a  class="btn btn-mini btn-warning" onclick="mergeOne('<s:property value="#st.index"/>')" >引入</a>
						<s:if test="#details.id!=null"> 
								<input type="hidden" name="socialBenefit.socialBenefitDetails[${st.index}].status" value="${details.status}">
								<a  class="closesocialBenefitItem btn btn-mini btn-inverse"><s:if test="#details.status ==1">停用</s:if><s:if test="#details.status ==0">启用</s:if></a>
						</s:if>
						<s:else>
							<a  class="btn btn-mini btn-danger"  onclick="subSocialBenefit(this,'<s:property value="#st.index"/>')" removeIndex="<s:property value="#st.index"/>" >删除</a>
						</s:else>
						<a class="btn btn-mini btn-info"  onclick="showDetail('${st.index}')">显示详细</a>
					</td> 
					</tr>
