...
<s:form action="socialBenefitBaseApply_save.action" theme="simple" id="editForm"  name="editForm" >
	 		...
	 		<p><label>社保类型名称*：</label><s:textfield cssClass="{validate:{required:true,messages:{required:'社保类型名称不能为空'}}} " id="socialBenefit.name" name="socialBenefit.name" /></p>
				...
				<p><label>社保所属月：</label><s:select name="socialBenefit.attributeMonth" id="attributeMonth" list="attributeMonthList"   headerKey="" headerValue="请选择" listKey="id" listValue="bdcode"></s:select></p>
				 ...
				 <p><label>&nbsp;</label>
					...
					<%-- <a class="uploadfiles btn"   url="socialBenefitBaseApply_toUploadFile.action" uploadfileHiddenValue="ssbFile" ><i class="icon-download-alt"></i>附件&nbsp;<span class="badge badge-info">${uploadFileNum }</span></a> --%>
    ...
    <div id="collapseTwo" class="accordion-body collapse">
      <div class="accordion-inner">
		...
		<div class="btn-toolbar ">
					<div class="btn-group">
					<a class="open btn btn-small btn-info" href="javascript:show(1)"><i class=" icon-folder-open  icon-white"></i>显示</a> 
					<a class="collapsed btn btn-small btn-info" href="javascript:show(0)"><i class="icon-folder-close  icon-white"></i>收起</a> 
					</div>
					<div class="btn-group">
					<a class="import btn btn-warning btn-small" href="javascript:merge()"><i class="icon-download-alt  icon-white"></i>全部引入</a> 
					</div>
					<div class="btn-group">
					<a class="addItem btn btn-primary btn-small" href="javascript:addSsbItemNew()"><i class="icon-plus  icon-white"></i>添加险种</a>
					</div>
		</div>
		<div id="editlist" >
...
<script type="text/javascript">
		$(function(){
			//启用和停用
			$("a.closesocialBenefitItem").die().live('click', function() {
				var _this = $(this);
			...
			function addSsbItemNew() {
				...
				$.ajax({
					url : "socialBenefitBaseApply_addItemNew.action",
					async : false,
					data : data,
					type : "POST",
					dataType : "html",
			...
			function merge() {
				...
				var data = $("#editForm").serialize();
				$.ajax({
					url : "socialBenefitBaseApply_merge.action",
					async : false,
					data : data,
					type : "POST",
					dataType : "html",
		...
		$("#province").change(function(){
			 ...
			 $.ajax({
					url : "socialBenefitBaseApply_queryCityByProvince.action?city.id="+checkValue,
					async : false,
					dataType : "JSON",
					type : "POST",
					cache: false,
