		   	...
		   	<tr>
				...
				<td align="center">
				       <c:if test="${method == 'manage' }">
						<a  class="btn btn-mini btn-info page_detail" href="socialBenefitBaseApply_detail.action?socialBenefit.id=${entry.id}">详细</a>
						<AT:AuthorityTag actionName="socialBenefitBaseApply_edit.action">
						<a  class="btn btn-mini btn-primary page_edit" href="socialBenefitBaseApply_edit.action?socialBenefit.id=${entry.id}">修改</a>
						</AT:AuthorityTag>
						<AT:AuthorityTag actionName="socialBenefitBaseApply_changeStatus.action">
							<c:if test="${entry.status == 2}">
								<a class="btn btn-mini page_status" href="socialBenefitBaseApply_changeStatus.action?socialBenefit.id=${entry.id}&socialBenefit.status=0">停用</a>
								<c:if test="${entry.createDate == entry.modifyDate}">
									<a class="btn btn-mini page_status" href="socialBenefitBaseApply_changeStatus.action?socialBenefit.id=${entry.id}&socialBenefit.status=1">新增审批</a>
								</c:if>
								<c:if test="${entry.createDate != entry.modifyDate}">
									<a class="btn btn-mini page_status" href="socialBenefitBaseApply_changeStatus.action?socialBenefit.id=${entry.id}&socialBenefit.status=1">修改审批</a>
								</c:if>								
							</c:if>
							<c:if test="${entry.status == 0}">
								<a class="btn btn-mini page_status" href="socialBenefitBaseApply_changeStatus.action?socialBenefit.id=${entry.id}&socialBenefit.status=2">启用</a>
							</c:if>
						...
						<c:if test="${method == 'view' }">
							<a  class="btn btn-mini page_detail" href="socialBenefitBaseApply_detail.action?socialBenefit.id=${entry.id}">详细</a>
						</c:if>
						
						<c:if test="${method == 'create' }">
						<a  class="btn btn-mini page_detail" href="socialBenefitBaseApply_detail.action?socialBenefit.id=${entry.id}">详细</a>
						<AT:AuthorityTag actionName="socialBenefitBaseApply_edit.action">
						<a   class="btn btn-mini page_edit" href="socialBenefitBaseApply_edit.action?socialBenefit.id=${entry.id}">修改</a>
						</AT:AuthorityTag>
						</c:if>
						...
						<AT:AuthorityTag actionName="socialBenefitBaseApply_changeStatus.action">
							<c:if test="${entry.status == 2}">
								...
								<%-- <a class="status pagetableoperate ui-button ui-widget ui-state-default" href="socialBenefitBaseApply_changeStatus.action?socialBenefit.id=${entry.id}&socialBenefit.status=1"><span class="ui-icon ui-icon-unlocked"></span>审批通过</a>&nbsp;--%>
							</c:if>
						</AT:AuthorityTag>	
						</c:if>
				</td>
...
<c:set var="url" value="socialBenefitBaseApply_prepaging.action" scope="request"/>
