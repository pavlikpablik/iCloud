...
public class DateUtil {
     ...
     * @return String
     */
    public static String getDateFormatByPattern(java.util.Date date , String pattern) {
    	if(date == null){
    		return null;
    	}
     ...
     *  验证一个年月是否合法   YYYY/MM
     *
     * @return boolean 合法为true  反之 false
     */
    public static boolean validateYYYYMM(String yyyymm) {
        /**
         * 详细设计： 1.返回boolean 验证一个年月是否合法   YYYY/MM
         */
    	yyyymm = StringUtils.trimToNull(yyyymm);
    	if(yyyymm == null){
    		return false;
    	}
    	//确定含有/
    	...
    	//确定为****/***的格式
    	String[] args = yyyymm.split("/");
    	if(args.length != 2){
    		return false;
    	}
    	
    	String yearStr = StringUtils.trimToNull(args[0]);
    	String monthStr = StringUtils.trimToNull(args[1]);
     ...
     * 数组小标  0 代表年  1 代表月 
     */
    public static Integer[] getNowDateTimeYYYYmm(String yyyymm){
    	 if(!validateYYYYMM(yyyymm)){
    		 return null; 
    		
    	 }
    	 String[] args = yyyymm.split("/");
    	 Integer[] yms = new Integer[]{0 , 0};
    	 yms[0] = Integer.parseInt(args[0]);
    	 yms[1] = Integer.parseInt(args[1]);
    	 return yms;
